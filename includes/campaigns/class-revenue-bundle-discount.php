...
class Revenue_Bundle_Discount {
	...
	public function init() {
		...
		add_filter( "revenue_campaign_{$this->campaign_type}_cart_item_price", array( $this, 'cart_item_price' ), 10, 2 );
	...
	public function after_bundle_parent_added_to_cart( $cart_item_key, $cart_item_data, $product_id, $bundle_quantity ) {
		...
		if ( isset( $cart_item_data['revx_bundle_type'], $cart_item_data['revx_bundle_data'] ) && 'trigger' === $cart_item_data['revx_bundle_type'] ) {
			...
			if ( isset( $cart_item_data['revx_bundle_with_trigger'], $cart_item_data['revx_trigger_product_id'] ) && 'yes' == $cart_item_data['revx_bundle_with_trigger'] ) {

                $bundle_cart_data = ['revx_campaign_id'=>$cart_item_data['revx_campaign_id'],'revx_campaign_type'=>$cart_item_data['revx_campaign_type'],'revx_bundle_id'=>$cart_item_data['revx_bundle_id'],'revx_bundle_type'=>'offer','revx_bundle_data'=>$cart_item_data['revx_bundle_data'],'revx_bundled_by'=>$cart_item_key,'rev_is_free_shipping' =>$cart_item_data['rev_is_free_shipping'],'revx_min_qty'=>1];
			...
			foreach ( $bundler_offer_data as $offer ) {
				foreach ( $offer['products'] as $offer_product_id ) {
                    ...
                    $bundle_cart_data = ['revx_campaign_id'=>$cart_item_data['revx_campaign_id'],'revx_campaign_type'=>$cart_item_data['revx_campaign_type'],'revx_bundle_id'=>$cart_item_data['revx_bundle_id'],'revx_bundle_type'=>'offer','revx_bundle_data'=>$cart_item_data['revx_bundle_data'],'revx_bundled_by'=>$cart_item_key,'revx_min_qty'=>$offer[ 'quantity' ],'rev_is_free_shipping' =>$cart_item_data['rev_is_free_shipping']];
	...
	public function before_calculate_cart_totals( $cart_item, $campaign_id, $cart ) {
		...
		foreach ( $cart->get_cart() as $cart_item ) {
			...
			if ( is_array( $offers ) ) {
				...
				// $is_bundle_with_trigger_product = 'yes' == revenue()->get_campaign_meta($campaign_id,'is_bundle_with_trigger_product',true);
	...
	public function cart_item_price( $subtotal, $cart_item ) {
		...
		if ( self::is_bundle_parent_item( $cart_item ) ) {
			$subtotal = wc_price( self::get_parent_cart_item_price( $subtotal, $cart_item ) );
		}
		return $subtotal;
	}
	...
	public function remove_cart_item( $remove_item_key, $cart_item ) {
		...
		if ( self::is_bundle_parent_item( $cart_item ) && ! self::is_removing_bundle_parent_item( $remove_item_key ) ) {
			...
			foreach ( $children as $child_key ) {
				...
				$this->removing_parent_key = $remove_item_key;
