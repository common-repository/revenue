...
if ( is_array( $offers ) ) {
	...
	foreach ( $offers as $offer_index => $offer ) {
		...
		foreach ( $offered_product_ids as $product_index => $offer_product_id ) {
			...
			$is_last_product = $offer_index == ( $offer_length - 1 ) && $product_index == ( $product_count - 1 );
			...
			$containerClass       = 'revx-campaign-item' . ( 'list' == $view_mode ? '' : ' revx-campaign-text-content' );
			...
			<div class="<?php echo esc_attr( $containerClass ); ?>" data-product-id="<?php echo esc_attr( ! $offered_product->is_type( 'variable' ) ? $offer_product_id : '' ); ?>" style="<?php echo esc_attr( $is_tag_enabled ? $tagged_product_style : $product_style ); ?>">
				<?php
				if ( 'list' == $view_mode ) {
					echo wp_kses(
					...
					<div class="revx-campaign-text-content revx-justify-space-sm revx-full-width">
						<div class="revx-full-width">
							...
							<div class="revx-flex revx-quantity-addToCart-container revx-flex-wrap">
								...
								if ( ! ( $total_offer_products == 1 && $is_last_product ) ) {
									echo wp_kses(
										revenue()->get_template_part(
											'add_to_cart',
											array(
			...
			<?php
				if ( ( $total_offer_products == 1 && $is_last_product && 'list' == $view_mode) ) {
					echo wp_kses(
						revenue()->get_template_part(
							'add_to_cart',
							array(
...
$containerClass = 'revx-normal-discount  ' . ( 'list' == $view_mode ? 'revx-campaign-list revx-normal-discount-list' : 'revx-normal-discount-grid revx-campaign-grid' );
	...
	?>
		<div class="revx-campaign-container__wrapper" style="<?php echo esc_attr( $wrapper_style ); ?>">
			<div class="<?php echo esc_attr( $slierSelector ); ?>" >
				<?php echo wp_kses( revenue()->get_slider_icon( $generated_styles, 'left' ), revenue()->get_allowed_tag() ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				<div class="<?php echo esc_attr( $gridClass ); ?>" style="<?php echo esc_attr( $product_gap ); ?>">
					<?php
                        echo wp_kses( $output_content, revenue()->get_allowed_tag() );
					?>
				</div>
				<?php echo wp_kses( revenue()->get_slider_icon( $generated_styles, 'right' ), revenue()->get_allowed_tag() ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			</div>
		</div>
	<?php
}
