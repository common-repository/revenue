...
if ( is_array( $trigger_items ) ) {
	foreach ( $trigger_items as $idx => $trigger ) {
		...
		$is_last_product = $idx == count( $trigger_items ) - 1;
		...
		?>
		<div class="<?php echo esc_attr( $item_class ); ?>" style="<?php echo esc_attr( $is_list_view ? $product_style : '' ); echo esc_attr('align-items: flex-start;'); ?>">
			<?php
			if ( $is_list_view ) {
				echo wp_kses(
					...
					<div class="revx-full-width">
                        ...
                        echo wp_kses(
                            revenue()->get_template_part(
                                'badge_sticky',
                                array(
                                    'message'          => sprintf( 'Buy %s', $trigger_qty ),
                                    'generated_styles' => $generated_styles,
                                )
                            ),
                            revenue()->get_allowed_tag()
...
if ( is_array( $offer_products ) ) {
	...
	foreach ( $offer_products as $offer_index => $offer ) {
		...
		switch ( $offer_type ) {
			case 'percentage':
				$_save_message_tag = "Get $offer_value% OFF";
				break;

            case 'fixed_discount':
            case 'amount':

				$_save_message_tag = "Get ".wc_price($offer_value).' OFF';
				break;
			case 'fixed_price':
				$_save_message_tag = 'Get ' . wc_price( floatval( $regular_price ) - floatval( $offer_value ) ).' OFF';
				break;
			case 'no_discount':
				$_save_message_tag='';
				break;
			case 'free':
                $_save_message_tag = 'Get Free';
				break;
		...
		?>
		<div class="<?php echo esc_attr($is_list_view? $item_class:'' ); ?>" style="<?php echo esc_attr( $is_list_view ? $product_style: '' ) ; echo esc_attr( 'align-items: flex-start;' ) ?>">
			<?php
			if ( $is_list_view ) {
				echo wp_kses(
					revenue()->get_template_part(
...
$containerClass = 'revx-buyx-gety ' . ( 'list' == $view_mode ? 'revx-campaign-list revx-buyx-gety-list' : 'revx-buyx-gety-grid revx-campaign-grid' );
...
if ( $is_list_view ) {
	...
	<div class="revx-campaign-container__wrapper" style="<?php echo esc_attr( $wrapper_style ); ?>">
		...
		<div class="revx-total-price revx-campaign-text-content" style="<?php echo esc_attr( $total_price_style ); ?>">
			<?php
			echo wp_kses( revenue()->tag_wrapper( $campaign, $generated_styles, 'totalPriceText', 'Total', 'revx-total-price-title', 'h4' ), revenue()->get_allowed_tag() );
			?>
	...
	<div class="revx-campaign-container__wrapper" style="<?php echo esc_attr( $wrapper_style ); ?>">
		<?php
		if ( $is_tag_enabled ) {
			echo wp_kses( revenue()->get_template_part( 'badge_tag',['generated_styles' => $generated_styles, 'current_campaign' => $campaign] ), revenue()->get_allowed_tag() );
			echo wp_kses( revenue()->get_template_part( 'badge_shape',['generated_styles' => $generated_styles, 'current_campaign' => $campaign] ), revenue()->get_allowed_tag() );
		}
		?>
		<div class="revx-flex">
...
revenue()->floating_container($campaign, $generated_styles, $output_content, $containerClass,false, $placement );
