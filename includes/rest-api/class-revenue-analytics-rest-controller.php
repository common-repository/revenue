...
class Revenue_Analytics_REST_Controller extends WP_REST_Controller {
	...
	public function get_total_stats( $request ) {
		...
		$intervalDays = $date_start->diff( $date_end )->format( '%a' );
		...
		switch ( $preset ) {
			...
			default:
				...
				$previous_date_start = ( clone $date_start )->modify( '-' . ( $intervalDays ) . ' days' );
				$previous_date_end   = ( clone $date_end )->modify( '-' . ( $intervalDays ) . ' days' );
				break;
		}
	...
	public function get_most_performing_stats( $request ) {
		...
		// 	SUM(CASE WHEN order_stats.parent_id = 0 THEN 1 ELSE 0 END) as orders_count,
		...
		$sql = "SELECT
            ...
            ORDER BY
                total_sales DESC;";  // Sort by total_sales in descending order

			$per_page = isset($request['per_page'])?intval(sanitize_text_field($request['per_page'])):10;
			$current_page = isset($request['paged']) ? absint(sanitize_text_field( $request['paged'] )) : 1; // Current page number
		...
		return rest_ensure_response( ['paginations' => $paginations, 'data' => array_values( $data ) ] );
	}
	...
	public function get_recent_campaigns_stats( $request ) {
		...
		foreach ( $current_results as $current ) {
			...
			if ( $trend['previous_add_to_cart_count'] != 0 ) {
			...
			if ( $trend['previous_conversion_rate'] != 0 ) {
	...
	// public function get_recent_campaigns_stats( $request ) {
	// 	$nonce = isset( $request['security']) ? sanitize_key($request['security']) : '';
	// 	if (!wp_verify_nonce($nonce, 'revenue-dashboard')) {
	// 		return new WP_Error('revenue_rest_nonce_error', __('Nonce Verification Failed!', 'revenue'), ['status' => 403]);
	// 	}

	// 	global $wpdb;

	// 	// Get date parameters
	// 	$from_date = !empty($request['range_query']['from']) ? sanitize_text_field($request['range_query']['from']) : '';
	// 	$to_date = !empty($request['range_query']['to']) ? sanitize_text_field($request['range_query']['to']) : '';

	// 	if (empty($from_date) || empty($to_date)) {
	// 		return new WP_Error('revenue_rest_invalid_date_range', __('Invalid date range provided!', 'revenue'), ['status' => 400]);
	// 	}
	...
	// 	$per_page = max(1, intval($request['per_page'] ?? 10));
	...
	// 			campaigns.campaign_status AS status,
	// 			campaigns.campaign_placement AS page,
	// 			DATE(revenue.date) AS date,
	// 			SUM(CASE WHEN order_stats.parent_id = 0 THEN 1 ELSE 0 END) AS orders_count,
	// 			SUM(order_stats.total_sales) AS total_sales,
	// 			SUM(revenue.impression_count) AS impression_count,
	// 			SUM(revenue.add_to_cart_count) AS add_to_cart_count,
	// 			CASE
	// 				WHEN SUM(revenue.impression_count) > 0 THEN
	// 					(SUM(CASE WHEN order_stats.parent_id = 0 THEN 1 ELSE 0 END) / NULLIF(SUM(revenue.impression_count), 0)) * 100
	// 				ELSE 0
	// 			END AS conversion_rate,
	// 			-- Previous period metrics
	// 			SUM(CASE WHEN revenue.date < %s THEN CASE WHEN order_stats.parent_id = 0 THEN 1 ELSE 0 END END) AS previous_orders_count,
	// 			SUM(CASE WHEN revenue.date < %s THEN order_stats.total_sales END) AS previous_total_sales,
	// 			SUM(CASE WHEN revenue.date < %s THEN revenue.impression_count END) AS previous_impression_count,
	// 			SUM(CASE WHEN revenue.date < %s THEN revenue.add_to_cart_count END) AS previous_add_to_cart_count
	// 		FROM
	// 			{$wpdb->prefix}revenue_campaigns AS campaigns
	// 		LEFT JOIN (
	// 			SELECT order_id, meta_value AS campaign_id
	// 			FROM {$wpdb->prefix}wc_orders_meta
	// 			WHERE meta_key = '_revx_campaign_id' AND meta_value IS NOT NULL
	// 		) AS orders ON campaigns.id = orders.campaign_id
	// 		LEFT JOIN {$wpdb->prefix}wc_order_stats AS order_stats ON (order_stats.order_id = orders.order_id OR order_stats.parent_id = orders.order_id)
	// 		LEFT JOIN {$wpdb->prefix}revenue_campaign_analytics AS revenue ON revenue.campaign_id = campaigns.id
	// 		WHERE
	// 			DATE(revenue.date) BETWEEN %s AND %s
	// 		GROUP BY
	// 			campaigns.id, campaigns.campaign_name, campaigns.campaign_status, campaigns.campaign_placement
	// 		LIMIT %d OFFSET %d
	// 		",
	...
	// 	$growth_trends = array_map(function($current) {
	// 		$previous_orders_count = $current->previous_orders_count ?? 0;
	// 		$previous_total_sales = $current->previous_total_sales ?? 0;
	// 		$previous_impression_count = $current->previous_impression_count ?? 0;
	// 		$previous_add_to_cart_count = $current->previous_add_to_cart_count ?? 0;
	...
	// 			'impression_count'           => $current->impression_count,
	// 			'previous_impression_count'  => $previous_impression_count,
	// 			'add_to_cart_count'          => $current->add_to_cart_count,
	// 			'previous_add_to_cart_count' => $previous_add_to_cart_count,
	// 			'conversion_rate'            => $current->conversion_rate,
	// 			'order_growth'               => $previous_orders_count != 0 ? (($current->orders_count - $previous_orders_count) / $previous_orders_count) * 100 : 100,
	// 			'sales_growth'               => $previous_total_sales != 0 ? (($current->total_sales - $previous_total_sales) / $previous_total_sales) * 100 : 100,
	// 			'impression_growth'          => $previous_impression_count != 0 ? (($current->impression_count - $previous_impression_count) / $previous_impression_count) * 100 : 100,
	// 			'add_to_cart_growth'         => $previous_add_to_cart_count != 0 ? (($current->add_to_cart_count - $previous_add_to_cart_count) / $previous_add_to_cart_count) * 100 : 100,
	// 			'conversion_rate_growth'     => $previous_orders_count != 0 ? (($current->conversion_rate - ($previous_orders_count > 0 ? ($current->previous_orders_count / $previous_orders_count) * 100 : 0)) / ($previous_orders_count > 0 ? ($current->previous_orders_count / $previous_orders_count) * 100 : 1)) * 100 : 100,
	// 		];
	// 	}, $results);
	...
	// 		"
	// 		SELECT COUNT(DISTINCT campaigns.id)
	// 		FROM {$wpdb->prefix}revenue_campaigns AS campaigns
	// 		LEFT JOIN {$wpdb->prefix}revenue_campaign_analytics AS revenue ON revenue.campaign_id = campaigns.id
	// 		WHERE DATE(revenue.date) BETWEEN %s AND %s
	// 		",
	// 		$date_start,
	// 		$date_end
	// 	));
		...
		if ( $from_date && $to_date ) {
			...
			FROM
				{$wpdb->prefix}revenue_campaigns rc
						INNER JOIN
				{$wpdb->prefix}wc_orders_meta co ON rc.id = co.meta_value
						LEFT JOIN
	...
	private function fetch_and_process_data( $request, $start_date, $end_date, $data_keys, $have_order_stats_keys, $have_campaign_stats_keys, $is_datewise ) {
		...
		if ( $is_campaign_typewise ) {
			...
			foreach ( $data as $key => $val ) {
				...
				$data[$key]['contribute'] = $total_sales != 0 ? number_format((float)(($val['total_sales'] / $total_sales) * 100), 2, '.', '') : number_format(0, 2, '.', '');
			}
			$results = $data;
		}
	...
	/**
	 * Retrieves campaign analytics data based on the provided request parameters and date range.
	 *
	 * This method constructs and executes an SQL query to fetch campaign analytics data.
	 * It supports filtering by date range, campaign selection, and aggregation by date or campaign type.
	 * The data is aggregated and grouped based on the specified request parameters and returned as an array of results.
	 *
	...
	// 	$query = $wpdb->prepare( "SELECT `$select_clause` FROM {$wpdb->prefix}revenue_campaign_analytics `$join` WHERE date BETWEEN %s AND %s", $date_start, $date_end ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared

	// 	if ( $selected_campaign ) {
	// 		$query = $wpdb->prepare( "SELECT `$select_clause` FROM {$wpdb->prefix}revenue_campaign_analytics WHERE date BETWEEN %s AND %s AND campaign_id = %s ", $date_start, $date_end, $selected_campaign ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	// 	}
	...
	public function get_campaign_analytics_data( $request, $start = false, $end = false ) {
		...
		$query = $wpdb->prepare($query, $date_start, $date_end, $selected_campaign);
	...
	public function get_order_stats_data( $request, $start = false, $end = false ) {
		...
		if ( $is_datewise ) {
			...
			$select_clause  .= "DATE({$wpdb->prefix}wc_order_stats.date_paid) AS date, ";
		}
		if ( $is_campaign_typewise ) {
			...
			LEFT JOIN (
				SELECT
					...
					{$wpdb->prefix}wc_orders_meta.meta_value AS campaign_id
				FROM
					{$wpdb->prefix}wc_orders_meta AS {$wpdb->prefix}wc_orders_meta
				WHERE
					{$wpdb->prefix}wc_orders_meta.meta_key = '_revx_campaign_id'
					AND {$wpdb->prefix}wc_orders_meta.meta_value IS NOT NULL
		...
		if ( in_array( 'total_sales', $data_keys ) ) {
			$select_clause .= "SUM({$wpdb->prefix}wc_order_stats.total_sales) AS total_sales, ";
		}
		...
		$select_clause = rtrim( $select_clause, ', ' );
		$query         = $wpdb->prepare( " SELECT {$select_clause} FROM {$wpdb->prefix}wc_order_stats AS {$wpdb->prefix}wc_order_stats WHERE {$wpdb->prefix}wc_order_stats.status NOT IN ('wc-auto-draft', 'wc-trash', 'wc-pending', 'wc-failed', 'wc-cancelled', 'wc-checkout-draft') AND {$wpdb->prefix}wc_order_stats.date_paid BETWEEN %s AND %s AND {$wpdb->prefix}wc_order_stats.order_id IN ( SELECT order_id FROM {$wpdb->prefix}wc_orders_meta WHERE meta_key = '_revx_campaign_id' AND meta_value IS NOT NULL )", $date_start, $date_end ); //phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		if ( $selected_campaign ) {
			$query = $wpdb->prepare( "SELECT {$select_clause} FROM {$wpdb->prefix}wc_order_stats AS {$wpdb->prefix}wc_order_stats WHERE {$wpdb->prefix}wc_order_stats.status NOT IN ('wc-auto-draft', 'wc-trash', 'wc-pending', 'wc-failed', 'wc-cancelled', 'wc-checkout-draft') AND {$wpdb->prefix}wc_order_stats.date_paid BETWEEN %s AND %s AND {$wpdb->prefix}wc_order_stats.order_id IN ( SELECT order_id FROM {$wpdb->prefix}wc_orders_meta WHERE meta_key = '_revx_campaign_id' AND meta_value <> {$selected_campaign} )", $date_start, $date_end ); //phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		}

		if ( $is_campaign_typewise ) {
			$query = $wpdb->prepare( "SELECT {$select_clause} FROM {$wpdb->prefix}revenue_campaigns {$join} WHERE {$wpdb->prefix}wc_order_stats.status NOT IN ('wc-auto-draft', 'wc-trash', 'wc-pending', 'wc-failed', 'wc-cancelled', 'wc-checkout-draft') AND {$wpdb->prefix}wc_order_stats.date_paid BETWEEN %s AND %s", $date_start, $date_end ); //phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		}
	...
	public function generate_chart_data( $start, $end, $data, $keys ) {
		...
		foreach ( $keys as $key ) {
			$data[ $key ] = array_merge( $dateArray, isset( $data[ $key ] ) ? $data[ $key ] : array() );
		}
	...
	public function getLastNDaysGraphData( $days = 7, $defaultValue = 0 ) {
		 $dates      = array();
		$currentDate = new DateTime(); // Today's date

		for ( $i = 0; $i < $days; $i++ ) {
			$dates[ $currentDate->format( 'Y-m-d' ) ] = $defaultValue;
	...
	public function calculateCampaignTrend( $data ) {
		...
		foreach ( $data as $date => $value ) {
			if ( $currentValue === null ) {
				$currentValue = $value; // Set current value (today's value)
			} else {
				$sumPrevious += $value; // Accumulate sum of previous values
			}
		}

		// Determine trend and return boolean
		return $currentValue > $sumPrevious ? 'up' : 'down';
	}
